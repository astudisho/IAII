...
class Grafica(object):
	...
	def plot(self, event):
		...
		if event.button == 1:
			...
			self.vectoresPrueba.append( vE.VectorEntrenamiento( ( event.xdata, event.ydata ),1 ) )
			self.ax.plot(event.xdata, event.ydata, 'or')
		self.figure.canvas.draw()

	def plotPrueba(self, x, y, color, tam):
		self.ax.plot( x, y, color, ms = tam)
		self.figure.canvas.draw()